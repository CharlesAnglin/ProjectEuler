  ...
  def findGreatestProduct(amountOfDigits: Int, bigNumber: String) = {
    ...
    def findProduct(index: Int, SumTotal: Int, greatestSumTotal: Int, indexOfGreatestProduct: Int): BigInt = {
      if (index + amountOfDigits - 1 == bigNumber.length) {
        return calcProduct(indexOfGreatestProduct, amountOfDigits)
      }
      val newSumTotal = SumTotal - bigNumber(index - 1) + bigNumber(index + amountOfDigits - 1)
      if (newSumTotal > greatestSumTotal && hasNoZeros(index)) {   //ERROR DUE TO GREATER THAN: E.G. 10+2 = 12, 10*2=20, 6+6=12, 6*6=36
        println("index "+ index)
    ...
    def calcProduct(index: Int, numbersLeft: Int, accumulator: BigInt = 1): BigInt = {
      if (numbersLeft == 0) {
        accumulator
      } else {
